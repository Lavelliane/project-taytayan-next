					...
					<div className="absolute inset-0 text-white z-10 w-full flex h-full items-start justify-between bg-black/40 rounded-t-lg">
						<div className='flex w-full items-start justify-between mx-6 mb-4 mt-6 gap-4'>
							...
							<div className='bg-neutral-50 flex flex-col px-3 py-1 rounded-lg items-center'>
								<span className={`${categoryColor} uppercase font-bold text-sm`}>{formatTimestamp(trainingDate).split(' ')[1].slice(0, 3)}</span>
								<span className='font-bold text-2xl text-black'>{formatTimestamp(trainingDate).split(',')[1].trim().split(' ')[1]}</span>
							</div>
						</div>
					</div>
				</div>	
		...
		>
			...
			<div className='flex flex-col gap-2 mx-6 mb-4'>
				<h5 className='flex gap-3 items-center'>
					<svg className="min-w-5 w-5 h-5 text-gray-800 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
						<path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 4h12M6 4v16M6 4H5m13 0v16m0-16h1m-1 16H6m12 0h1M6 20H5M9 7h1v1H9V7Zm5 0h1v1h-1V7Zm-5 4h1v1H9v-1Zm5 0h1v1h-1v-1Zm-3 4h2a1 1 0 0 1 1 1v4h-4v-4a1 1 0 0 1 1-1Z"/>
					</svg>
					<span className='font-base text-xs lg:text-sm'>{trainingCenter}</span>
				</h5>
				<h5 className='flex gap-3 items-center'>
					<svg className='min-w-5 w-5 h-5 text-gray-800 dark:text-white'aria-hidden='true' xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 17 21'>
