  ...
  const [sortOption, setSortOption] = useState<string>('alphabetical'); // Default sorting option
	...
	const fetchEvents = async () => {
		try {
			...
			if (userStore.role === 'training_center') { // TODO: What is the equivalent of this in event_hosting?
				userEvents = userStore.myTrainings;
			} else if (userStore.role === 'general') {
				userEvents = userStore.eventsJoined;
			}

			const eventsRef = query(collection(db, 'networking'), where('eventId', 'in', userEvents));
			...
			if (userStore.eventsJoined.length > 0) {
				...
				eventsDoc.forEach((doc) => {
					const eventData: NetworkingEvent = { ...(doc.data() as NetworkingEvent) };
	...
	return (
		<main className='flex flex-col w-full px-4 md:px-8 lg:px-12 xl:px-24'>
			<section className='py-8'>
				<h1 className='font-inter font-semibold pb-4 text-lg text-dark'>My Events</h1>
				<div>
					<div className='pb-8'>
						<div className='flex gap-6 justify-between'>
							...
							{userStore.role === 'training_center' && <AddEventButton />}
						</div>
            ...
            {/* TODO: Create MyEventCard */}
					...
					{networkingEvents.length === 0 && <h1 className='text-center font-semibold'>No events created</h1>}
				</div>
			</section>
		</main>
	);
