		...
		<Modal dismissible show={learnMoreOpened} position='center' size='2xl' onClose={() => handleLearnMoreClose()} popup>
			...
			<Modal.Body className='flex flex-col gap-4 my-4'>
				<div className='flex gap-2 mt-4 items-center'>
					...
					/>
					<h1 className='text-sm lg:text-base font-bold'>{networkingEventData.eventName}</h1>
				</div>
				<p className='text-xs lg:text-sm text-gray-500'>{networkingEventData.eventDescription}</p>
				<div className='text-xs lg:text-sm font-bold'>{formatTimestamp(networkingEventData.eventDate)}</div>
				...
				<div className='text-sm lg:text-base font-bold'>
					...
					{parseFloat(networkingEventData.eventRegistration) === 0 ? (
						...
						<span className='text-red-600 font-bold'>{networkingEventData.eventRegistration}</span>
					)}
				</div>
				<div>
					<span className='text-sm lg:text-base font-bold'>Activities:</span>
					<ul className='list-disc text-gray-500 list-inside'>
						{networkingEventData.eventActivities.map((activity: string, index: number) => (
				...
				<div>
					...
					<ul className='list-disc text-gray-500 list-inside'>
						{networkingEventData.eventObjectives.map((objective: string, index: number) => (
							<li key={index} className='text-xs lg:text-sm'>
								{objective}
							</li>
						))}
				...
				<div className='flex flex-col gap-2'>
					<div className='flex gap-2 items-center'>
						...
						<SeeRegistrantsButton registrant={networkingEventData?.eventRegistrants} />
					</div>
					<FormatRegistrants registrant={networkingEventData?.eventRegistrants} />
				</div>
			</Modal.Body>
			<Modal.Footer>
